...
assembly_jar="$(find ${assembly_path} -name "*-assembly*.jar" -type f | uniq)"

# Translate the ~ to absolute paths
function tr_classpath { perl -pe "s@~@$HOME@g" "${sbt_path_root}/$@"; }
function mk_classpath { local IFS=':'; echo "$*"; }

export PYSPARK_PYTHON="$(which python${py_ver})"
export PYSPARK_DRIVER_PYTHON=$PYSPARK_PYTHON

# Setup python paths
_local_pypath="${_bsd_}/python:$(python${py_ver} -c 'import site; print(site.USER_SITE)')"
...
export PYSPARK_SUBMIT_ARGS="--driver-memory 4g --executor-memory 4g --py-files ${_submit_py_files} --jars ${_submit_jars} pyspark-shell"
